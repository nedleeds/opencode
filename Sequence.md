...
- **자동 동기화** — 사전 준비 단계가 없다. 처음 묻는 매뉴얼은 그 질문에서 ~5초 들여 받아오고, 이후로는 로컬이다.
  - 필요한 건 `codeload.github.com`과 `raw.githubusercontent.com` 접근뿐이다. **사내망이라도 GitHub이 열려 있으면 기본값 그대로 두면 된다.**
  - 프록시 환경도 그대로 동작한다 — 다운로드는 `curl`로 나가므로 `HTTP_PROXY`/`HTTPS_PROXY`와 시스템 CA를 따른다.
  - GitHub 자체가 막힌 경우에만 선택지가 갈린다: 사내 미러가 있으면 `HRBOOK_TARBALL_BASE`/`HRBOOK_BOOKINFOS_URL`을 돌려주고, 아무 경로도 없으면 `HRBOOK_AUTOSYNC=0`으로 매번 타임아웃을 기다리지 않고 즉시 실패시킨다.
